			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.amount >= toAmount then
                        ...
                        TriggerEvent('DiscordBot:ToDiscord', 'hile', "Dupe yapmaya çalıştı. Yapmaya çalıştığı item: " .. toItemData.name .. "   Sahte item sayısı: " .. toAmount .. "   Gerçek item sayısı: ".. toItemData.amount .. "   Değiştirdiği item: " ..fromItemData.name, src, Player.PlayerData.source)
					end
				else
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		...
		if price == nil then
			if shopType == "merry" then 
				price = exports["tgiann-base"]:merryWeatherPrice(itemInfo["name"])
			elseif shopType == "police" then 
				price = exports["tgiann-base"]:policePrice(itemInfo["name"])
			elseif shopType == "ambulance" then
				price = exports["tgiann-base"]:ambulancePrice(itemInfo["name"])
			else
				price = exports["tgiann-base"]:itemPrice(itemInfo["name"])
			end
		end
...
-- 			if prices == nil then
-- 				if shop == "merry" then 
-- 					prices = exports["tgiann-base"]:merryWeatherPrice(itemInfo["name"])
-- 				elseif shop == "police" then 
-- 					prices = exports["tgiann-base"]:policePrice(itemInfo["name"])
-- 				elseif shop == "ambulance" then 
-- 					prices = exports["tgiann-base"]:ambulancePrice(itemInfo["name"])
-- 				elseif shop == "Itemshop_Ed_Market" then
-- 					prices = exports["tgiann-base"]:esxkrshop(itemInfo["name"]) * 3
-- 				else
-- 					prices = exports["tgiann-base"]:itemPrice(itemInfo["name"])
-- 				end
-- 			end
