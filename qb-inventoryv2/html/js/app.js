    ...
    if (itemData != null && itemData.info != "") {
        if (itemData.type == "weapon") {
            ...
            $(".item-info-description").html(
                ...
                "</span></p><p><strong>Doğum T.: </strong><span>" +
                itemData.info.birthdate +
                "</span></p><p><strong>Cinsiyet: </strong><span>" +
                gender +
                "</span></p><p><strong>Köken: </strong><span>" +
                itemData.info.nationality +
                "</span></p>"
            );
        } else if (itemData.name == "rozet") {
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            $(".item-info-description").html(
                "</span></p><p><strong>İsim: </strong><span>" +
        ...
        start: function(event, ui) {
            ...
            if ( dragAmount == 0) {
                if (itemData.price != null) {
                    $(".ui-draggable-dragging").find(".item-slot-amount").html(+itemData.amount);
                    $(".ui-draggable-dragging").find(".item-slot-price").html('$'+itemData.price);
                    $(".ui-draggable-dragging").find(".item-slot-key").remove();
                } else {
                    $(".ui-draggable-dragging").find(".item-slot-amount").html(ra1derduzenleme(itemData.amount, itemData.weight));
                    $(".ui-draggable-dragging").find(".item-slot-key").remove();
                }
                ...
                if (itemData.price != null) {
                    $(this).find(".item-slot-amount").html(+itemData.amount);
                    $(this).find(".item-slot-price").html('$'+itemData.price);
                    $(".ui-draggable-dragging").find(".item-slot-amount").html(+itemData.amount);
                    $(".ui-draggable-dragging").find(".item-slot-price").html('$'+itemData.price);
                    $(".ui-draggable-dragging").find(".item-slot-key").remove();
                } else {
                    $(this).find(".item-slot-amount").html(ra1derduzenleme(itemData.amount - dragAmount, itemData.weight));
                    $(".ui-draggable-dragging").find(".item-slot-amount").html(ra1derduzenleme(dragAmount, itemData.weight));
                    $(".ui-draggable-dragging").find(".item-slot-key").remove();
                }
            } else {
                $(".ui-draggable-dragging").find(".item-slot-key").remove();
                $(this).find(".item-slot-amount").html(ra1derduzenleme(itemData.amount, itemData.weight));
                InventoryError();
            }
        },
        stop: function() {
            ...
            // $(this).css("background", "var(--item-slot-color)");
            var itemData = $(this).data("item");
    ...
    if (fromData !== undefined) {    
        ...
        if((toData != undefined || toData != null) && toData.name == fromData.name && !fromData.unique) {
            ...
            if (fromData.amount == $toAmount) {
                ...
                if ($toSlot < 7 && $toInv.attr("data-inventory") == "player") {
                ...
                if ($fromInv.attr("data-inventory").split("-")[0] == "itemshop") {
                    ...
                    if ($fromSlot < 7 && $fromInv.attr("data-inventory") == "player") {
...
function durubalityControl(item) {
    var durubality = 100
    var QualityColor = "rgb(98, 233, 117)";
...
Inventory.Open = function(data) {
    // $("#player-inv-label").text(data.playername + " (" + data.id + ")"); 
    $("#player-inv-label").html('Oyuncu Envanteri - '+ data.id)
    totalWeight = 0;
    totalWeightOther = 0;
    inventoryOpen = true;
    ...
    for (let i = 1; i < clotheSlots+1; i++) {
        slots = slots + '<div class="item-slot clothe-item-slot" data-slot="' + (playerInvSlot + i) + '"><div class="item-slot-img"></div><div class="item-slot-label">ANAN</div></div>'
    }
...
Inventory.ToggleHotbar = function(data) {
    if (data.open) {
        if (inventoryOpen == false) {
            let slots = ""
            for(i = 1; i < 6; i++) {
                slots = slots + '<div class="item-slot" data-zhotbarslot="' + i + '"><div class="item-slot-key">' + i + '</div><div class="item-slot-img"></div><div class="item-slot-label">&nbsp;</div></div>'
            }
            ...
            $.each(data.items, function(i, item){
                if (item != null) {
                    ...
                    $(".z-hotbar-inventory").find("[data-zhotbarslot=" + item.slot + "]").html('<div class="item-slot-img"><img src="images/' + item.image + '" alt="' + item.name + '" /></div><div class="item-slot-amount"><p><div class="item-ra1der-amount">'+item.amount+'x</div><div class="item-ra1der-weight"><i class="agirlik fas fa-weight-hanging"></i> &nbsp;'+ ((item.weight * item.amount) / 1000).toFixed(1) +'</div></p></div>' + ItemLabel);
...
var requiredTimeout = null;
    ...
    $itembox.html('<div id="itembox-action"><p>' + type + '</p></div><div id="itembox-label"><p>'+data.item.label+'</p></div><div class="item-slot-img"><img src="images/' + data.item.name + '.png" alt="' + data.item.name + '" /></div>');
